# Jon Martin

**Founding PM Who Builds From Zero**

Bay Area | Open to Remote | jonalexjames@gmail.com | (650) 627-6352 | portfolio.jonnymartin.blog

---

## Summary

Three-time zero-to-one builder. First PM at Genies (85-person startup, $150M raised). Co-founded Frame Story, a game studio where I incorporated the company, built the financial model, and am executing a pre-seed raise. Built Pottery Friends solo from idea to 150-member beta. Fifteen years of product experience across startups and scale — but I do my best work when there is no playbook and the job is to create one.

---

## Experience

### Founding Product Manager — Genies
**Feb 2021 – May 2022**

- First PM hire at an 85-person startup with no product infrastructure, celebrity partnerships to manage, and everything to prove
- Built the Creator Ecosystem from concept to launch — the product story that helped secure $150M in funding with partners including Gucci and GIPHY
- Launched an e-commerce storefront generating $100K in weekly sales within months of launch
- Shipped mobile app with GIPHY integration, scaling to 3K early users; defined GTM strategy that grew alpha community to 1K users

### Fractional Product Manager (AI) — Swob
**Nov 2024 – Present**

- Embedded as the product function for a pre-funding, pre-revenue AI hiring platform — no team, no infrastructure, everything to build from zero
- Building and shipping features end-to-end using AI-assisted development: prototyping with Claude Code, validating with users, pushing production code
- Shaped the AI strategy: killed predictive features with insufficient data and legal exposure, redirected engineering toward automations that delivered immediate user value
- Prototyped swipe-to-match hiring flow; tested with 3 restaurant managers — time to shortlist dropped from days to under 2 minutes
- Recommended splitting a monolithic 5-feature app into separate apps per persona after user testing revealed cognitive overload

### Co-Founder & Director of Product — Frame Story
**Feb 2025 – Present**

- Co-founded a collaborative game studio with recently laid-off developers and designers; modeled after Finji and Raw Fury
- Led business formation end-to-end: LLC, banking, payment processing, contributor agreements, IP assignment
- Built 5-year financial model and investor materials: $3M cap SAFE, 1-to-12 title portfolio strategy
- Executing $100K pre-seed raise via SAFEs, grants (Epic MegaGrant, Wings Fund), and crowdfunding
- Designed phased GTM for first title Cluck: Steam Next Fest, indie festivals, creator partnerships, Kickstarter
- Leading product on Cluck with a 10+ person team through milestone-driven Steam release

### Founder / PM / Dev — Pottery Friends
**Feb 2025 – Present**

- Identified an unowned vertical after embedding at a pottery studio 6 days/week for a year — no one has built purpose-built software for craft studio communities
- Built the full product ecosystem solo: native mobile app, marketing site, analytics dashboards, internal docs
- Grew early access beta to 150 members with consistent weekly usage; running structured PMF validation
- Killed scope ruthlessly: cut a planned marketplace after user interviews, pivoted the home screen when data contradicted the hypothesis, reduced 40 onboarding quests to 8

### Senior PM — Treasure DAO
**Apr 2024 – Nov 2024**

- Owned the full product stack: marketplace, quest system, developer platform, and new chain launch
- Launched Quest v2 and Verified Actions — engagement up 30%, data integration time down 40%
- Shipped refactored web app, marketplace, testnet, and developer portal — active developers up 35%

### Senior Technical PM — Mythical Games
**May 2022 – Dec 2023**

- Shipped a cross-title game services platform: retention up 20%, revenue up 15%
- Defined the game services roadmap by identifying gaps competitors missed: live ops tooling, unified player identity, cross-title progression

### Digital Product Manager — AAA
**Mar 2018 – Aug 2020**

- Launched API-driven mobile app to 6M members; saved $2M/year by shifting support to self-service
- Only PM reporting directly to executive leadership; weekly C-Suite strategy presentations

### Earlier Roles
**2009 – 2017** | Jam City ($50M product, 1M+ DAU) | Bandai Namco (PAC-MAN, 10M+ installs) | Big Fish Games (analytics pipeline from scratch) | Flow State Media (Dir. of Product, 8 months revenue growth) | SUPERLABS (VR, acquired by Zynga) | Zynga (FarmVille, built Mafia Wars first mobile feature)

---

## Core Competencies

**0-to-1 Execution:** Market discovery / MVP scoping / user research / beta programs / PMF validation

**GTM & Fundraising:** Go-to-market strategy / investor materials / SAFE structures / financial modeling / pitch narratives

**Cross-Functional Leadership:** Engineering, design, and ops alignment / PRDs engineers want to read / sprint execution without micromanaging

**Technical Fluency:** React / Next.js / TypeScript / Supabase / Claude Code / PostHog — engineers don't have to dumb things down, I've shipped production code with the same stack

---

## Education

University of Southern California
